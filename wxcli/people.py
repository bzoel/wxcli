...
def list(
    ...
    if filter_emaildomain is not None:
        people = [
            person for person in people if f"@{filter_emaildomain}" in person.emails[0]
        ]

    table = table_with_columns(
        ["Name", "Email", "Created", "Invite Pending", "Login Enabled"], title="People"
    )
    for p in people:
        table.add_row(
            p.displayName,
...
def create_team_membership_by_emaildomain(
    team_name: str, org_name: str, people_emaildomain: Optional[str] = None
):
    """
    Add 1 or more people to a room or team
    """
    people_args = {"max": 1000}
    # Find org
    orgs = api.organizations.list()
    try:
        org = next(org for org in orgs if org.displayName == org_name)
    ...
    if people_emaildomain is not None:
        people = [
            person for person in people if f"@{people_emaildomain}" in person.emails[0]
        ]
    else:
    ...
    for p in people:
        if not any(tm for tm in team_memberships if p.id == tm.personId):
            api.team_memberships.create(teamId=team.id, personId=p.id)
            console.log(
                f" [green][bold]{p.displayName}[/bold] has been added to [bold]{team.name}[/bold]"
            )
        else:
            console.log(
                f" [bold]{p.displayName}[/bold] is already a member of [bold]{team.name}[/bold]"
            )


@app.command()
def create_room_membership_by_emaildomain(
    room_name: str, org_name: str, people_emaildomain: Optional[str] = None
):
    """
    ...
    for p in track(people, console=console):
        if not any(m for m in memberships if p.id == m.personId):
            api.memberships.create(roomId=room.id, personId=p.id)
            console.log(
                f" [green][bold]{p.displayName}[/bold] has been added to [bold]{room.title}[/bold]"
            )
        else:
            console.log(
                f" [bold]{p.displayName}[/bold] is already a member of [bold]{room.title}[/bold]"
            )
