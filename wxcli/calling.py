...
def update_user_cid(
    ...
    selected_type: Optional[CallerIdSelectedType] = typer.Option(
        None, help="Type of number"
    ),
    name_policy: Optional[CallerIdNamePolicy] = typer.Option(
        None, help="Caller ID name policy"
    ),
    ...
    person = person[0]
    console.print(
        f"[green]Found user [bold]{person['displayName']}[/bold] with extension [bold]{person['extension']}[/bold]"
    )
    ...
    api_req(
        f"people/{person['id']}/features/callerId", method="put", json=updated_values
    )


{"selected": "CUSTOM", "customNumber": "8594257735"}
...
def list_caller_ids(
    ...
    table = table_with_columns(
        ["Person", "Extension", "Caller ID Type", "Caller ID Number"],
...
def update_location_allcids(
    location_name: str = typer.Argument(..., help="Webex Calling Location Name"),
    caller_id: str = typer.Argument(
        "LOCATION_NUMBER", help="Caller ID | LOCATION_NUMBER | DIRECT_LINE"
    ),
    org_id: str = typer.Option(None, help="Organization ID"),
):
    """
        ...
        if custom_number is None:
            ...
            if org_id is not None:
                ...
                caller_id_resp = api_req(
                    f"people/{person['id']}/features/callerId",
                    method="put",
                    json={"selected": caller_id, "customNumber": custom_number},
                )
...
def all_phone_apply_config(
    ...
    # Find callerId for each person
    table = table_with_columns(
        ["Person", "Extension", "Phone Model", "MAC Address", "Type"],
        title="User Phone Info",
    )
    for person in track(people):
        if org_id is not None:
            devices = api_req(
                f"telephony/config/people/{person['id']}/devices",
                params={
                    "orgId": orgId,
                },
            )
        ...
        for device in devices["devices"]:
            ...
            if device["type"] == "PRIMARY":
                if org_id is not None:
                    result = api_req(
                        f"telephony/config/devices/{device['id']}/actions/applyChanges/invoke",
                        method="post",
                        params={
                            "orgId": orgId,
                        },
